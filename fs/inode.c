...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos) {
    ...
    // 第 inode_no 号 I 节点相对于 inode_table_lba 的字节偏移量
    uint32_t off_size = inode_no * inode_size;
    // 第 inode_no 号 I 节点相对于 inode_table_lba 的扇区偏移量
    ...
    if (left_in_sec < inode_size) {
        // 若扇区内剩下的空间不足以容纳一个 inode，必然是 I 节点跨越了 2 个扇区
...
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {
    ...
    // 以下 inode 的 3 个成员只存在于内存中
    // 现在将 inode 同步到硬盘，清掉这三项即可
    pure_inode.i_open_cnts = 0;
    pure_inode.write_deny = false;
    pure_inode.inode_tag.prev = pure_inode.inode_tag.next = NULL;

    // io_buf 是用于硬盘的 io 缓冲区
...
struct inode* inode_open(struct partition* part, uint32_t inode_no) {
    ...
    // inode 位置信息会存入 inode_pos，包括 inode 所在扇区地址和扇区内字节偏移量。
    inode_locate(part, inode_no, &inode_pos);
    ...
    if (inode_pos.two_sec) {    // 考虑跨扇区的情况
        ...
        // i 节点表是被 partition_format 函数连续写入扇区的，所以下面可以连续读出来
        ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, 2);
    ...
    // 因为一会可能要用到此 inode，故将其插入到队首便于提前检测到
...
void inode_delete(struct partition* part, uint32_t inode_no, void* io_buf) {
    ...
    char* inode_buf = (char*)io_buf;
    if (inode_pos.two_sec) {    // inode 跨区则读入两个扇区
        // 将原硬盘上的内容先读出来
        ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, 2);
        // 将 inode_buf 清 0
...
void inode_release(struct partition* part, uint32_t inode_no) {
    ...
    // b. 如果一级间接块表存在，将其 128 个间接块读到 all_block[12~]，并释放一级间接块表所占的扇区
    ...
    // c. inode 所有的块地址都已经收集到了 all_blocks 中，然后遍历一遍再逐个回收就 ok 了
    block_idx = 0;
