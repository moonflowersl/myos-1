...
static void partition_format(struct partition* part) {
    ...
    // 2. 将块位图初始化并写入 sb.block_bitmap.lba
    // 初始化块位图 block_bitmap
    ...
    // 2.1 先将位图最后一字节到其所在的扇区的结束全置为 1，即超出实际块数的部分直接置为己占用
    memset(&buf[block_bitmap_last_byte], 0xff, last_size);
    // 2.2 再将上一步中覆盖的最后一字节内的有效位重新置 0
    uint8_t bit_idx = 0;
...
int32_t sys_mkdir(const char* pathname) {
    ...
    if (inode_no != -1) {   // 如果找到了同名目录或文件，失败返回
        ...
        uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
        // 先判断是否把 pathname 的各层目录都访问到了，即是否在某个中间目录就失败了
        if (pathname_depth != path_searched_depth) {    // 说明并没有访问到全部的路径，某个中间目录不存在
...
// 创建文件或目录需要创建相关的多个资源，若某步失败则会执行下面操作回滚
rollback:
...
// 读取目录 dir 的第一个目录项，成功后返回其目录项地址，到目录尾时或出错时返回 NULL
...
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr, void* io_buf) {
    ...
    // 目录中的 ".." 中包括父目录 inode 编号，".." 位于目录的第 0 块
...
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf) {
    ...
    // 填充 all_blocks，将该目录所占的扇区全部存入 all_blocks 中
    uint8_t block_idx = 0;
...
// 当 buf 为 NULL 时，有操作系统分配存储工作路径的空间并返回地址
// 失败则返回 NULL
...
// 在 buf 中填充文件结构相关信息，成功时返回 0，失败时返回 -1
