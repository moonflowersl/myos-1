...
extern intr_handler intr_entry_table[IDT_DESC_CNT];     // 声明引用定义在 kernel.S 中的中断函数入口数组
...
static void exception_init(void) 
{
    ...
    for (i = 0; i < IDT_DESC_CNT; i++)
    {
        // idt_table 数组中的函数是在进入中断后根据中断向量号调用的，默认为 general_intr_handler
...
void idt_init()
{
    ...
    // // 加载 idt
    // uint64 idt_operand = ((sizeof(idt) - 1) | (uint64_t)((uint32_t)(idt << 16)));
