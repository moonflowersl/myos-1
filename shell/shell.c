...
// 用来记录当前目录，是当前目录的缓冲，每次执行 cd 命令时都会更新此内容
char cwd_cache[64] = {0};

// 输出提示符
void print_prompt(void) {
    printf("[moonflower@localhost %s]$ ", cwd_cache);
}
...
static void cmd_execute(uint32_t argc, char** argv) {
    if (!strcmp("ls", argv[0])) {
       ...
       buildin_rm(argc, argv);
        ...
        if (pid) {  // 父进程
            int32_t status;
            int32_t child_pid = wait(&status);  // 此时子进程若没有执行 exit，my_shell 会被阻塞，不再相应键入的命令
