...
void init_thread(struct task_struct* pthread, char* name, int prio)
{
    ...
    if (pthread == main_thread)
        // 由于吧 main 函数也封装成一个进程，并且它一直是运行的，故将其直接设为 TASK_RUNNING
    ...
    pthread->stack_magic = 0x20161005;  // 自定义模数
    pthread->ticks = prio;
    pthread->pgdir = NULL;
...
{
    // main 线程早已运行
    // loader.S 中进入内核时的 mov esp,0xc009f000 就是为其预留 pbc 的
    // 因此 pcb 地址为 0xc009e000，不需要通过 get_kernel_page 另分配一页
    main_thread = running_thread();
    init_thread(main_thread, "main", 31);

    // main 函数是在当前线程，当前线程不在 thread_ready_list 中
